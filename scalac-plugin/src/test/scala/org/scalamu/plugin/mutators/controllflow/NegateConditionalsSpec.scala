...
class NegateConditionalsSpec extends SingleMutationSpec {
  override def mutation: Mutator = NegateConditionals

  "NegateConditionals" should "negate conditional operators" in withScalamuCompiler { (global, reporter) =>
    val code =
      """
        |object Foo {
        |  val a = 123
  ...
  it should "not negate controllflow operators on unsupported types" in withScalamuCompiler { (global, reporter) =>
    val code =
      """
        |object Foo {
        |  case class A(i: Int) {
