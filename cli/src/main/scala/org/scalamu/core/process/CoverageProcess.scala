...
object CoverageProcess extends Process[ValidatedNel[SuiteFailure, SuiteCoverage]] {
  ...
  override type Configuration = (CoverageProcessConfig, Path, Map[String, Array[Byte]])
  ...
  override def readConfigurationFromParent(
    ...
    } yield (config, outputDir, classes)
  ...
  override def run(
    ...
    val (config, invocationDataDir, classes) = configuration
    val reader                               = new InvocationDataReader(invocationDataDir)
    val inMemoryClassLoader                  = new InMemoryClassLoader(classes)
    
//    withContextClassLoader(inMemoryClassLoader) {
    Thread.currentThread().setContextClassLoader(inMemoryClassLoader)
      reader.clearData()
